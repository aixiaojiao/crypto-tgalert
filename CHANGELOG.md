...
#### **📋 界面显示优化**
- **blacklist_list界面**: 系统级过滤从显示数量改为显示具体代币符号列表
- **yellowlist_list界面**: 系统级过滤从显示数量改为显示具体代币符号列表
- **mute_list界面**: 保持原有详细界面不变

**修改位置**:
- `src/services/telegram/commands/BlacklistCommandHandler.ts:144`
- `src/services/telegram/commands/YellowlistCommandHandler.ts:144`

#### **🚫 系统级过滤规则统一**
- **风险代币**与**已下架代币**现在采用完全相同的过滤规则
- **历史数据收集**: 风险代币不再保留历史数据，与已下架代币保持一致
- **交易对过滤**: 两类代币都从交易对列表中完全移除

**修改位置**: `src/config/tokenLists.ts:75-94`
