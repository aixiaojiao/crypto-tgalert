...
/**
 * 黑名单管理命令处理器
 * 支持命令: /black <symbol> | /black_list | /black_remove | /black_clear
 */
export class BlacklistCommandHandler extends BaseCommandHandler {
  readonly command = 'black';
  readonly description = '管理个人黑名单 - /black <symbol> /black_list /black_remove /black_clear';
  ...
  private async handleSystemList(): Promise<CommandResult> {
    try {
      ...
      message += '• 使用 /black <symbol> 添加个人黑名单';
