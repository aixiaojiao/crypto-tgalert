...
#### **🚨 重大Bug发现与修复**
- **关键问题发现**: 用户反馈UB、DAM等代币日志显示处理但未收到推送通知
- **问题分析**: 排除1分钟重复过滤机制，锁定过滤器内部逻辑
- **根因定位**: AdvancedFilterManager中黄名单代币错误设置`allowed: false`
- **影响评估**: 15个YELLOWLIST_TOKENS代币(UB, DAM, YALA, GPS, ZORA, PTB, Q, AIO, AVNT, SAPIEN, JELLYJELLY, F, BB, ACE, PUMPBTC)完全无法推送
- **修复实施**: 将黄名单代币的allowed从false改为true，保持⚠️风险标识显示
- **验证方案**: TypeScript编译通过，等待重启验证推送恢复

**🎯 优化成果**: 过滤命令系统架构更加简洁明确，功能无重复，用户体验一致性显著提升。关键推送bug得到及时发现和修复，系统可靠性进一步提升。
